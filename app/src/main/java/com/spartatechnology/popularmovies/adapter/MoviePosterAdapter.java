...
public class MoviePosterAdapter extends ArrayAdapter<Movie> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ImageView imageView;
        if (convertView == null) {
            imageView = (ImageView) LayoutInflater.from(getContext()).inflate(R.layout.movie_item, parent, false);
        } else {
            imageView = (ImageView) convertView;
        }

        final String imageUri = PopularMoviesConstants.PICASSO_URL + movie.getPosterPath();
        Picasso.with(getContext()).load(imageUri).placeholder(R.drawable.movie_placeholder).into(imageView);
        return imageView;
    }
}
